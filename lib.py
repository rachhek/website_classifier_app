...
class WebsiteClassifierTool:
    def __init__(self):
        try:
            ...
            if os.path.exists(tfidf_vectorizer_file):
                ...
                print(
                    colored.blue(
                        "loaded pre-computed tfidf vectorizer object from {}".format(
                ...
                print(
                    colored.red("Can't find file at {}".format(tfidf_vectorizer_file))
                )
            ...
            if os.path.exists(log_reg_model_file):
                ...
                print(
                    colored.blue(
                        "loaded pre-trained logistic regression model object from {}".format(
                            log_reg_model_file
                        )
                    )
                )
            else:
                print(colored.red("Can't find file at {}".format(log_reg_model_file)))
    ...
    def predict(self, website_url):
        ...
        try:
            ...
            website_text_content = " ".join(res[columns_to_combine].astype(str))
            print(
                "\n Sample scraped text: {}... \n".format(
            ...
            vectorized_text = self.tfidf_tokenizer.transform([website_text_content])
            print(
                "\n Words with highest TF-IDF values: {}\n".format(
                    colored.yellow(self.get_top_tf_idf_words(vectorized_text))
                )
            )
            ...
            class_prob = {
                class_name: prob
                for class_name, prob in zip(self.log_reg_model.classes_, class_prob[0])
            }
            if result:
                return {
                    "predicted_class": result[0],
                    "prob_distribution": sorted(
                        class_prob.items(), key=itemgetter(1), reverse=True
                    ),
                }
        except Exception as e:
            print(colored.red(str(e)))
